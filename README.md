...
![HackerRank Orchestrate 2026](https://img.shields.io/badge/HackerRank-Orchestrate_2026-blue?style=for-the-badge)
...
The agent processes `support_tickets.csv` through a strictly typed 7-stage pipeline:
...
### 2. Installation
Clone the repository and install dependencies inside the `code/` directory:
```bash
...
**Run against the sample CSV (for testing/development):**
```bash
bun run start:sample
```

**Run against the main production CSV (evaluation):**
```bash
bun run start
```
...
- **Ticket Traces:** Written to `code/logs/ticket-XXX.json`. Open these to see the exact queries, scores, and decision logic for any ticket.
- **Run Summary:** Written to `code/logs/summary.json` containing accuracy, duration, and confidence averages.
- **Chat Transcript Log:** Written to `$HOME/hackerrank_orchestrate/log.txt` (or `%USERPROFILE%\hackerrank_orchestrate\log.txt` on Windows), tracking agent tooling interactions during development.
...
This project fulfills all criteria outlined in `problem_statement.md` and `evalutation_criteria.md`:
- **Terminal-based:** Fully orchestrated via the CLI.
- **Grounded Responses:** Responses strictly adhere to the provided local corpus (`data/`) without external live web calls.
- **Escalation Protocol:** System explicitly escalates high-risk, sensitive, or out-of-bounds cases to human reviewers instead of hallucinating answers.

*Submission consists of this repository code, the generated `output.csv`, and the developer transcript `log.txt`.*